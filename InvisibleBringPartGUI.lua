-- BERLIAN_BRINGPART FINAL (GUI + Slider + Safe Pull)
-- Hanya menarik benda, karakter sendiri aman
...
local function applyForceToPart(part, targetPos)
    ...
    bv.P = math.clamp(tonumber(strengthSlider.Text) or PULL_STRENGTH,100,1e7)
    local direction = (targetPos - part.Position).unit
    local distance = (targetPos - part.Position).magnitude
    -- Tambah offset agar mengelilingi karakter, bukan langsung menabrak
    local offset = Vector3.new(math.sin(tick()*distance)*10,0,math.cos(tick()*distance)*10)
    bv.Velocity = direction * math.min(600,distance*2) + offset
    bv.Parent = part
